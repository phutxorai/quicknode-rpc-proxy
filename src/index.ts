...
export default {
	async fetch(request: Request, env: Env) {
		...
		if (upgradeHeader || upgradeHeader === 'websocket') {
			return await fetch(`https://${env.QUICKNODE_SUBDOMAIN}.quiknode.pro/${env.QUICKNODE_API_KEY}/`, request)
		}


		const { pathname, search } = new URL(request.url)
		const payload = await request.text();
		const proxyRequest = new Request(`https://${env.QUICKNODE_SUBDOMAIN}.quiknode.pro/${env.QUICKNODE_API_KEY}/`, {
